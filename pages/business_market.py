...
db = st.session_state.dbClient["hackharvard"]
contracts = (
    rec_collection.find({"traded_from": {"$ne": st.session_state.username + "@gmail.com"}})
    .sort([("timestamp", -1)])
    .limit(10)
)
...
if available_credits:
    ...
    if st.button("Purchase Selected Credit"):
        ...
        total_cost = int(selected_credit["REC_credits_traded"]) * selected_credit["price_of_contract"]

        index = next(
            i
            for i, credit in enumerate(contracts)
        ...
        rec_collection.update_one({"_id": selected_credit["_id"]},
                                  {"$set": {"traded_to": st.session_state.get("username", "") + '@gmail.com',
                                            "is_offer_in_market": False}})
        company_collection.update_one({"name": st.session_state['username']},
                                      {"$set": {"money": company['money'] - total_cost, "carbon_balance":
                                          company['carbon_balance'] + selected_credit["REC_credits_traded"]}})
        st.rerun()
else:
